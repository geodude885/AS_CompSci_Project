...
class Creature(object):
    ...
    def __init__(self, num, parent=None):
        ...
        try:
            ...
            while self.fitness <= 0:
                ...
                if typeMute == False:
                    self.sze = parent.sze + \
                        random.uniform(-MUTATIONMAX,
                                       MUTATIONMAX) ** MUTATIONRATE
                    self.per = parent.per + \
                        random.uniform(-MUTATIONMAX,
                                       MUTATIONMAX) ** MUTATIONRATE
                    self.stren = parent.stren + \
                        random.uniform(-MUTATIONMAX,
                                       MUTATIONMAX) ** MUTATIONRATE
                    self.int = parent.int + \
                        random.uniform(-MUTATIONMAX,
                ...
                if typeMute == True:
                    self.sze = parent.sze + 5 * \
                        random.uniform(-MUTATIONMAX,
                                       MUTATIONMAX) ** MUTATIONRATE
                    self.per = parent.per + 5 * \
                        random.uniform(-MUTATIONMAX,
                                       MUTATIONMAX) ** MUTATIONRATE
                    self.stren = parent.stren + 5 * \
                        random.uniform(-MUTATIONMAX,
                                       MUTATIONMAX) ** MUTATIONRATE
                    self.int = parent.int + 5 * \
                        random.uniform(-MUTATIONMAX,
                                       MUTATIONMAX) ** MUTATIONRATE
                    self.end = parent.end + 5 * \
                        random.uniform(-MUTATIONMAX,
                                       MUTATIONMAX) ** MUTATIONRATE
    ...
    def fitness(self):
        ...
        fitness = (environment.foodDensity + self.sze) * environment.warmth * \
            environment.waterDensity / + 1 + self.per + \
            (self.end * 2 * environment.weather)
        fitness += self.stren * environment.foodDensity + \
            self.int * environment.waterDensity
        return fitness
...
def naturalSelection(generation):
    ...
    if rep == True:
        ...
        # basically it makes a list of chances of survival those kills them
        # with low values

        # kills half of the generation population based on fitness value
        for numberKilled in range(int(len(generation) / 2)):
            toKill = int(funcs.mapBetween(
                1 - (random.random() ** DEATHRANDOMNESS), 0, 1, 1, len(generation) - 1))
...
def reproduction(generation):
    ...
    if rem == True:
        global overallCreatureNum
        # repopulates generation with creatures based on survivors from
        # previous generation
        for parentNum in range(len(generation)):
            generation.append(
                Creature(overallCreatureNum, generation[parentNum]))
            overallCreatureNum += 1
