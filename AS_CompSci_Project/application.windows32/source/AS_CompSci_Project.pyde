...
#Runs at the application startup
def setup():
    fullScreen()
    background(21, 34, 56)
    noiseDetail(12)
...
def draw():
    ...
    #Runs the simulation as fast as possible
    if genASAP == True:
    ...
    #Runs simulation every other frame, with natural selection shown
    if showGenASAP == True:
...
#Used to show all the GUI elements and display everything on screen. Not called every frame to reduce hardware demand
def updateScreen():
    clear()
    background(21, 34, 56)
...
"""
    ...
    if creatScreen:
        ...
        text("GENERATION: " + str(genCount), 1100, 30)
    ...
    #Draws the information screen
        ...
        fill(200)
        textFont(font, 52)
        text("EVOLUTION SIMULATOR", 800, 60)
        text("GENERATION: " + str(genCount), 840, 186)
...
#Runs whenever a key is pressed, used for the hotkeys in application
def keyPressed():
    
    #Changes between creature and information screen
    if key == TAB:
        global creatScreen
        creatScreen = not creatScreen
        updateScreen()
        
    #Runs single full generation iteration
    if key in ("I", "i"):
        global generation, genCount, topFit, botFit, avgFit, types
        genetic_creature.updateEnvironment(warmthSlider.value, foodSlider.value, waterSlider.value, weatherSlider.value)
        generation = genetic_creature.genIteration(generation)
    ...
    #Toggles on/off running simulation as fast as possible
    if key in ("U", "u"):
        global genASAP
        genASAP = not genASAP
        
    #Toggles on/off showing simulation every other frame with natural selection
    if key in ("Y", "y"):
        if showGenStep == 0:
            global showGenASAP
...
#Run on every mouse click. There are only 3 buttons that actually require this.
def mouseClicked():
...
#Displays info when mouse is over the info button
def dispInfo():
...
#Displays the tutorial, shows text depending on genCount value
def infoScreenText():
    textFont(font, 32)
    fill(200)
...
"FITNESS"), then you'll observe this species become dominant!""", 40, 680)
        ...
        textFont(font, 32)
        text("""Congratulations on completing the tutorial! Have fun watching
evolution in action!""", 400, 600)
        textFont(font, 24)
